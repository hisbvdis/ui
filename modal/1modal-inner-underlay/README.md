...
- Обозначает, что нажатие на элемент должно открывать модальное окно
- Является ориентиром для общего обработчика события, который обрабатывает клики на любые такие элементы
- Убрав атрибут с элемента, позволяет отключить открытие модального окна (например, чтобы написать свою логику его обработки)

**Атрибут `data-modal-trigger`**
- Является ориентиром для поиска триггера, открывшего окно
