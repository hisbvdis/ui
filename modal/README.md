...
### Стилизация
**Скрытие/показ окна (через display:none)**
- Не позволяет использовать переходы (анимации)
- ВАЖНО: зато текстовое содержимое скрытых окон не ищется через браузер, что лучше

**Скрытие/показ окна (через .srOnly)**
- ПЛОХО: текстовое содержимое скрытых окон ищется через браузер, что плохо
- *Для скрытия окна:*
